...
class TestFileStorage(unittest.TestCase):
    ...
    def setUp(self):
        ...
        del_list = list(self.storage.all().keys())
        for key in del_list:
    ...
    def test_new_object_added_to_objects(self):
        ...
        for obj in self.storage.all().values():
            temp = obj
            self.assertIs(temp, obj)

    def test_all_returns_dict(self):
        """ __objects is properly returned as a dict """
        new = BaseModel()
    ...
    def test_reload_from_empty_file_raises_value_error(self):
    ...
    def test_reload_from_nonexistent_file_does_nothing(self):
